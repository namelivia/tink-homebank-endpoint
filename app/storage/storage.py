...
class TokenStorage(Storage):
    ...
    # TODO: tink-http-python expects FileNotFound exceptions to be raised
    def retrieve_refresh_token(self) -> str:
        if self.refresh_token is None:
            raise FileNotFoundError("Refresh token is not set")
        return self.refresh_token

    def retrieve_authorization_code(self) -> str:
        if self.authorization_code is None:
            raise FileNotFoundError("Authorization code is not set")
